...
• **絶対条件**：2枚目と3枚目は**必ず50文字以上**。これは絶対に守ること。50文字未満は出力禁止。
• 目標レンジ：1枚目は30〜35文字、2枚目と3枚目は70〜75文字に必ず収める。75文字を超えることは厳禁。
• **最大値を目指す**：文字数は常に最大値に近づける努力を必須とする。1枚目は35文字、2・3枚目は75文字を目標とする。
• **文字数が足りない場合の対応（必須）**：
  1. まず素材内の未使用ディテールを活用
  2. それでも足りない場合は**登録済み知識から会社の特徴や理念を自然に織り込む**
  3. 例：「〜してくれた」→「〜してくれて、この職場の温かさを感じた」
  4. 例：「〜と思った」→「〜と思って、ここで働けることに感謝した」
  5. **絶対に50文字未満で出力しない**。必ず内部資料から補完して50文字以上にする。
